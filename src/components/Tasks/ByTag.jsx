...
import { getTasksByTag, postTask } from '../../client/tasks'
...
export default function TasksByTag() {
    ...
    const tagID = quertParams.get('id');
    const tagHeading = quertParams.get('heading');
    const [tagTasksCount, settagTasksCount] = useState(0);
    ...
    const countQuery = useQuery(
        {
            queryKey: ["tasksCount", "tags", tagID],
            queryFn: async () => {
                const response = await getTasksByTag(tagID);
                if (response.error) {
    ...
    const {
        ...
        queryKey: ["tasks", "tags", tagID],
        queryFn: async ({ pageParam = 1 }) => {
            const response = await getTasksByTag(tagID, pageParam, 20);
            if (response.error) {
                throw response.error;
            }
        ...
        getNextPageParam: (lastPage, allPages) => {
            ...
            return nextPage > Math.ceil(tagTasksCount / 20) ? undefined : nextPage;
        },
        staleTime: 5 * 60 * 1000
    });
    ...
    useEffect(() => {
        (async () => {
            if (countQuery.data) {
                settagTasksCount(countQuery.data);
            }
    ...
    return (
        ...
        <>
            ...
            <div className={`bg-gray-100 w-full h-fit my-3 mx-3 p-6 rounded-lg text-gray-700 relative`}>
                <h1 className='md:text-4xl text-3xl font ml-14'>{tagHeading}<span className='m-1 px-3 py-1 bg-gray-300 rounded-md '>{tagTasksCount}</span></h1>
