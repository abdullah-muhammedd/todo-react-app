...
import { getTasksToday, postTask } from '../../client/tasks'
...
export default function Today() {
    ...
    const countQuery = useQuery(
        {
            queryKey: ["tasksCount", "todayCount"],
            queryFn: async () => {
                const response = await getTasksToday();
                if (response.error) {
    ...
    const {
        ...
        queryFn: async ({ pageParam = 1 }) => {
            const response = await getTasksToday(pageParam, 20);
            if (response.error) {
                throw response.error;
            }
        ...
        getNextPageParam: (lastPage, allPages) => {
            ...
            return nextPage > Math.ceil(todayCount / 20) ? undefined : nextPage;
        },
        staleTime: 5 * 60 * 1000
    });
    ...
    return (
        ...
        <>
            ...
            <div className={`bg-gray-100 w-full h-fit my-3 mx-3 p-6 rounded-lg text-gray-700 relative`}>
                <h1 className='md:text-4xl text-3xl font ml-14'>Today<span className='m-1 px-3 py-1 bg-gray-300 rounded-md '>{todayCount}</span></h1>
